...
| Galaxy S23 Ultra | Mixed reports: one user said it works, another reported no charge and no HDMI | Low | Power brick sensitivity; seating/fit may matter | Full DeX plus app visibility plus read-only IN responses | 2026-05-16 local test: charges, HDMI works, DeX launches, and the app sees `0x04B4:0x5210` plus `0x0BDA:0x8152`. Widened read-only vendor IN probes later found non-empty 16-byte responses for request `0x05` on `0x0BDA:0x8152`. Fan remained silent throughout. |
...
- Read-only probe expansion result from 2026-05-16: `0x0BDA:0x8152` returned 16-byte all-zero payloads for vendor IN request `0x05` with request types `0xC0` and `0xC1`; `0x04B4:0x5210` returned no data in the tested request window.
